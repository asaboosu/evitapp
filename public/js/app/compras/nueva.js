...
var eventoCompletar = function(row) {
    ...
    try {
        if (productos[0].categoria) {
            $.ajax({
                url: 'http://' + window.location.hostname + "/" + "compra/nueva",
                type: "POST",
                data: {
                    'productos': JSON.stringify(productos)
                }
            }).then(function(response) {
                window.location.pathname = "/compra/completar/" + response;

            });
        }

    } catch (error) {
        alerta.success('error', 'Error', 'No se han agregado productos.');
    }
    ...
    try {
        if (productos[0].categoria) {
            $.ajax({
                ...
                var print_area = window.open("http://" + window.location.hostname + "/compra/ticketcliente/" + response);
                print_area.print();
                print_area.onfocus = function() { setTimeout(function() { print_area.close(); }, 500); }
...
var eventoCantidad = function() {
    $('.cantidad').each(function() {
        ...
        var productoNuevo = {
            ...
            'total': total,
            'cantidad': input.val(),
            'eliminar': "<button class='btn btn-danger eliminar'>X</button>",
...
var eventoGuardar = function() {
    ...
    $.ajax({
        ...
        success: function() {
            try {
                alerta.success('success', 'Exito', 'Se ha guardado la compra.', function() {
...
$('body').on('change', '#codProducto', function() {
    var id = useInput('#codProducto', true);
    if (id == false) {
        alerta.default(
            'error',
...
//aqui el programador se dio cuenta que era urgente utilizar un framework completo y no jquery xd